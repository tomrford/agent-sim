...
Use this when you want another coding agent to migrate an older simulation DLL to the current `agent-sim` capabilities (CAN transport, DBC overlay usage, env wiring, and shared-state channels).
...
2. Optional CAN ABI (`sim_can_get_buses`, `sim_can_rx`, `sim_can_tx`)
3. Optional shared-state ABI (`sim_shared_get_channels`, `sim_shared_read`, `sim_shared_write`)
4. Runtime env wiring via `agent-sim.toml` (`env start`)
5. DBC overlay usage (`can.<bus>.<signal>` reads/writes) with env-level DBC loading
...
6. Validate end-to-end with targeted commands and tests.
7. Provide a concise migration report:
   - what changed,
...
- When `dbc` is present, `env start` should attach and then load DBC automatically for each member session.
...
2. Confirm buses:
   - `agent-sim --session ecu1 can buses`
3. Confirm DBC-projected signal is readable (after traffic exists):
   - `agent-sim --session ecu1 get can.internal.engine_rpm`
4. Write DBC signal:
   - `agent-sim --session ecu1 set can.internal.throttle_position=25.0`
5. Validate shared channel:
   - `agent-sim --session ecu2 shared get sensor_feed.*`
